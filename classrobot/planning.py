...
class Planning:
    ...
    def compute_traj_time(self, d, v_tool, a_max):
        ...
        # d = distance , v_tool = speed end-effcotr , a_max = accleration input
        # --- Sanity checks ---
        if d < 0:
        ...
        # # d_ramps represents the distance required to decelerate (or accelerate) the tool to a stop (or from rest) 
        # based on the maximum acceleration (a_max) and the current velocity of the tool (v_tool).
        # This formula comes from kinematics: d = v^2 / (2 * a), but here it assumes a factor of 2 is already accounted for elsewhere.
